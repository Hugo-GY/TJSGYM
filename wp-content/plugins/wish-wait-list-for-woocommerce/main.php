...
class BeRocket_Wish_List extends BeRocket_Framework {
    ...
    function __construct () {
        $this->info = array(
            ...
            'full_name'   => 'Wishlist and Waitlist for WooCommerce',
            'norm_name'   => 'Wish/Wait List',
            'price'       => '',
    ...
    <div style="width: 250px;">
        ...
        if( $this->check_framework_version() ) {
            if ( $this->init_validation() ) {
                ...
                add_action( 'woocommerce_shortcode_before_products_loop', array($this, 'before_shortcode'), 10, 1 );
                add_action( 'woocommerce_shortcode_after_products_loop', array($this, 'after_shortcode'), 10, 1 );
    ...
    function shortcode_atts_products($out, $pairs, $atts) {
        if( ! empty($atts['brwwl']) && $atts['brwwl'] == 'close' ) {
            $out['brwwl'] = true;
        }
        return $out;
    }
    function before_shortcode($attr) {
        if( ! empty($attr['brwwl']) ) {
            add_action('woocommerce_before_shop_loop_item', array($this, 'shortcode_remove_button'), 1000);
        }
    }
    function shortcode_remove_button() {
        global $product;
        $element = '<div class="berocket_ww_product" data-id="'.$product->get_ID().'">';
        $element .= '<a class="berocket_ww_remove" href="#remove"><i class="fa fa-times"></i></a>';
        $element .= '</div>';
        echo $element;
    }
    function after_shortcode($attr) {
        if( ! empty($attr['brwwl']) ) {
            remove_action('woocommerce_before_shop_loop_item', array($this, 'shortcode_remove_button'), 1000);
        }
    }
