...
assert_contains "$ROOT_DIR/club-kit.html" "TJ's Thermal Bottle"
assert_contains "$ROOT_DIR/club-kit.html" "TJ's Teddy Bear"
assert_contains "$ROOT_DIR/club-kit.html" 'id="kit-support"'
if rg -Fq 'Ordering help' "$ROOT_DIR/club-kit.html"; then
  echo "Did not expect Ordering help link to remain" >&2
  exit 1
fi
if rg -Fq 'Full Range' "$ROOT_DIR/club-kit.html"; then
  echo "Did not expect Full Range card to remain" >&2
  exit 1
fi
CARD_COUNT=$(rg -o 'class="kit-product-card"' "$ROOT_DIR/club-kit.html" | wc -l | tr -d ' ')
if [ "$CARD_COUNT" -ne 16 ]; then
  echo "Expected 16 product cards, found $CARD_COUNT" >&2
  exit 1
fi
assert_contains "$ROOT_DIR/club-kit.html" '<a href="club-kit.html"    class="nav-link active">Club Kit</a>'
assert_contains "$ROOT_DIR/index.html" '<a href="club-kit.html"    class="nav-link">Club Kit</a>'
